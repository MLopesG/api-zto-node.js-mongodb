...
const usersModel = require('./../model/usersAdminModel.js');
...
module.exports.add = (req, res) => {
	...
	if (!errors.isEmpty()) {
		...
		usersModel.verifyEMAIL(connectMongoSchemas.createUserAdmin,req.body.email, (error,result) => {
			if (result.length > 0) {
				res.json({
					status: false,
					message: 'Email já sendo utilizado.'
				});
			} else {
